...
func (n *Node) Start(listenAddr string, bootstrapNodes []string) error {
	...
	// bootsrap the network with a list of already know nodes
	// in the network
...
func (n *Node) validatorLoop() {
	...
	n.logger.Infow("starting validator loop",
		"pubKey", n.PrivateKey.Public(),
		"blockTime", blockTime)
	...
	for {
		...
		n.logger.Debugw("time to create a new block",
			"lenTx",
			len(txx))
...
func (n *Node) addPeer(c proto.NodeClient, v *proto.Version) {
	...
	// handle the logic whether we decide to accept or drop
	// the incoming node connection

	n.peers[c] = v

	// Connect to all peers in tge received list of peers
	if len(v.PeerList) > 0 {
...
func (n *Node) bootstrapNetwork(addrs []string) error {
	for _, addr := range addrs {
		...
		n.logger.Debugw("dialing remote nodes",
			"we", n.ListenAddr,
			"remote", addr)
...
func makeNodeClient(listAddr string) (proto.NodeClient, error) {
	c, err := grpc.Dial(listAddr, grpc.WithInsecure())
	if err != nil {
		return nil, err
	}
